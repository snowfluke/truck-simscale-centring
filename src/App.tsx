...
import { Road, TruckScale, Trucks, Sensors, InteractiveFloor, FacilityDecorations, Explosions, DeadTrucks } from './components/SceneEntities';
import { StatsPanel } from './components/StatsPanel';
import { useStore, useSensorCalculations, TOTAL_L, getTruckTransform } from './store';
...
function EasterButton() {
     ...
     const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
          ...
          if (file) {
               ...
               if (boxTrucks.length > 0) {
                    const randomTruck = boxTrucks[Math.floor(Math.random() * boxTrucks.length)];
     ...
     return (
          <>
               ...
               <button disabled={disabled} onClick={() => inputRef.current?.click()} className={clsx("p-1 px-3 text-sm shadow-sm border rounded flex items-center gap-1", disabled ? "bg-slate-100 text-slate-400 border-slate-200" : "bg-white text-emerald-600 border-slate-300 hover:bg-emerald-50 hover:border-emerald-300")}>
                    🐇 Easter
               </button>
          </>
     );
...
export default function App() {
  ...
  useEffect(() => {
     ...
     if (autoMode) {
         const SPACING = TOTAL_L / Math.max(1, profiles.length);
         let mixedProfiles: any[] = [];
         let boxP = profiles.filter(p => p.hasBox !== false).sort(() => Math.random() - 0.5);
         let noboxP = profiles.filter(p => p.hasBox === false).sort(() => Math.random() - 0.5);
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
        ...
        const mappedTrucks = state.trucks.map((t, i) => {
            let tState = { ...t };
            const trackZ = -30 + tState.d; 
            const truckZFront = trackZ + tState.length / 2;
            const truckZRear = trackZ - tState.length / 2;

            if (tState.logoUrl && tState.flowState === 'weighing' && Math.abs(trackZ) < 5) {
                 const tform = getTruckTransform(tState.d);
                 state.addExplosion(tform.position.x, tform.position.y, tform.position.z);
            ...
            const scaleOccupied = state.trucks.some((otherT, tIdx) => {
                ...
                return !['approaching', 'driving', 'facility_exit_approaching', 'facility_exit_holding', 'facility_exit_leaving'].includes(otherT.flowState);
            });
            ...
            switch (tState.flowState as any) {
                ...
                case 'weighing_complete':
                   tState.timer += dt;
                   if (tState.timer > 1.5) { // Delay before gate opens
                       tState.timer = 0;
                ...
                case 'exit_opening':
                   tState.timer += dt;
                   if (tState.timer > 1.5) { // Gate opened
                       tState.timer = 0;
            ...
            if (!autoModeRef.current && i === 0) {
                ...
                if (state.trucks.length > 1) {
                    const queueD = 17.5 - tState.length / 2;
                    const distToQueue = queueD - tState.d;
                    const aheadT = i === 0 ? state.trucks[state.trucks.length - 1] : state.trucks[i - 1];
                    ...
                    if (scaleOccupied && distToQueue >= -0.5 && distToQueue < 30) {
                         targetSpeed = Math.min(targetSpeed, Math.max(0, distToQueue * 0.4));
                         shouldLimit = true;
                    }

                    if (gap < 8.0) {
                         targetSpeed = Math.min(targetSpeed, Math.max(0, (gap - 1.0) * 0.5));
                         shouldLimit = true;
                    }
                    ...
                    if (shouldLimit) {
                         if (targetSpeed <= 0.1) {
                              ...
                              backward = false;
                              tState.speed *= 0.5;
                              if (gap <= 0.5 || (scaleOccupied && distToQueue <= 0.1)) tState.speed = 0;
                         } else {
            ...
            const isOutsideGate = ['approaching', 'exiting', 'driving'].includes(tState.flowState);
            ...
            const isCorner = (tState.d > 55 && tState.d < 96) || (tState.d > 146 && tState.d < 182.83) || (tState.d < 5);
            ...
            if (autoModeRef.current && (tState.flowState === 'weighing' || tState.flowState === 'weighing_complete') && Math.abs(trackZ) <= 0.05) {
                 tState.speed = 0;
            }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown' || e.key === 's') keyState.current.backward = true;
    };
    const handleKeyUp = (e: KeyboardEvent) => {
      if (e.key === 'ArrowUp' || e.key === 'w') keyState.current.forward = false;
      if (e.key === 'ArrowDown' || e.key === 's') keyState.current.backward = false;
    };
      ...
      <div className="absolute top-4 left-4 z-20 flex gap-2">
         ...
         <select 
            ...
            onChange={(e) => {
               ...
               if (p) updateActiveTruck({ selectedProfileId: p.id, width: p.width, height: p.height, length: p.length });
            }}
         ...
         <button onClick={resetTruck} className="p-1 px-3 bg-white text-sm shadow-sm border border-slate-300 hover:bg-rose-50 text-slate-600 hover:text-rose-600 rounded flex items-center gap-1">
            <RotateCcw size={14} /> Reset
         </button>
         <EasterButton />
      </div>

      <div className="flex-1 h-full relative overflow-hidden" style={{ backgroundImage: 'radial-gradient(#cbd5e1 1px, transparent 1px)', backgroundSize: '20px 20px' }}>
         <Canvas 
         ...
         <div className="absolute bottom-6 left-6 bg-white/90 p-2 rounded-lg flex flex-col gap-2 shadow-[0_4px_12px_rgba(0,0,0,0.05)] border border-slate-200 z-20">
            <button 
                onMouseDown={() => { btnState.current.forward = true; }} 
                onMouseUp={() => btnState.current.forward = false}
                onMouseLeave={() => btnState.current.forward = false}
                onTouchStart={() => { btnState.current.forward = true; }}
            ...
            <button 
                onMouseDown={() => { btnState.current.backward = true; }} 
                onMouseUp={() => btnState.current.backward = false}
                onMouseLeave={() => btnState.current.backward = false}
                onTouchStart={() => { btnState.current.backward = true; }}
