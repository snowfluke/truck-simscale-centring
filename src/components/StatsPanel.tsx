...
export function StatsPanel() {
  ...
  const maxSensorZ = sensors.reduce((maxZ, s) => Math.max(maxZ, s.z), -Infinity);
  ...
  const activeLeftCount = readings.filter(r => r.hit && r.sensor?.placement === 'left').length;
  const activeRightCount = readings.filter(r => r.hit && r.sensor?.placement === 'right').length;
  ...
  if (sensorCenter !== null && entryState === "FULLY IN") {
      ...
      if (topHits.length === 0) {
          ...
          const farthestBottom = Math.max(...bottomHits.map(r => Math.abs(r.sensor.z)));
  ...
  } else if (entryState === "IN") {
      instructorStatus = "guiding";
      instruction = "Truck not fully inserted. Keep driving forward.";
      cmd = "MOVE FORWARD";
  }
  ...
  } else if (entryState === "FULLY IN" && instructorStatus !== 'centered' && instructorStatus !== 'guiding') {
  ...
  if ((entryState === "IN" || entryState === "FULLY IN") && activeLeftCount > 0 && activeRightCount === 0) {
      cmd = "MOVE FORWARD";
  ...
  } else if ((entryState === "IN" || entryState === "FULLY IN") && activeRightCount > 0 && activeLeftCount === 0) {
      cmd = "MOVE BACKWARD";
      cmdBgClass = "bg-amber-500/90 border-l-4 border-amber-700";
      cmdTitleClass = "text-amber-200";
      cmdValueClass = "text-white";
  ...
  return (
    <>
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 pointer-events-none flex flex-col items-center gap-2">
         <button onClick={() => setInfoModal('goal')} className="bg-indigo-50/95 border border-indigo-200 text-indigo-700 px-5 py-2 rounded-full shadow-lg text-[11px] font-bold flex items-center gap-2 hover:bg-indigo-100 hover:scale-105 transition-all pointer-events-auto">
      ...
      <div className="absolute top-4 right-4 flex flex-col gap-2 z-10">
          <div className={`w-[240px] p-3 rounded shadow-lg transition-colors ${cmdBgClass}`}>
             <div className={`text-[10px] uppercase tracking-wider mb-1 ${cmdTitleClass}`}>DRIVER INSTRUCTION</div>
             <div className={`text-sm font-bold ${cmdValueClass}`}>{cmd}</div>
             <div className={`text-[11px] mt-2 ${cmdDetailsClass}`}>{instruction}</div>
          </div>

          <div className={`w-[240px] p-3 rounded shadow-lg transition-colors ${engineerBgClass}`}>
             <div className={`text-[10px] uppercase tracking-wider mb-1 ${engineerTitleClass}`}>ENGINEER GROUND TRUTH</div>
             <div className={`text-sm font-bold ${engineerValueClass}`}>{engineerCmd}</div>
             <div className={`text-[11px] mt-2 ${engineerDetailsClass}`}>{engineerInstruction}</div>
          </div>
      </div>
